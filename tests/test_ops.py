...
class TestOPSExpression(object):
    ...
    @pytest.mark.parametrize('equation,expected', [
        ('Eq(u.forward, u + 1)',
         ...
         '&(u[0][0][0]), "float", "ut0"), ops_decl_dat(block, 1, u_dim, u_base, u_d_m, '
         'u_d_p, &(u[1][0][0]), "float", "ut1")}\']'),
        ('Eq(u.forward, u + v.dx)',
         '[\'ops_dat u_dat[2] = {ops_decl_dat(block, 1, u_dim, u_base, u_d_m, u_d_p, '
         '&(u[0][0][0]), "float", "ut0"), ops_decl_dat(block, 1, u_dim, u_base, u_d_m, '
         'u_d_p, &(u[1][0][0]), "float", "ut1")}\','
        ...
        ('Eq(w1.forward, w1 + 1)',
         ...
         'w1_d_p, &(w1[0][0][0]), "float", "w1time0"), ops_decl_dat(block, 1, w1_dim, '
         'w1_base, w1_d_m, w1_d_p, &(w1[1][0][0]), "float", "w1time1")}\']'),
        ('Eq(w2.forward, w2 + v.dx)',
         '[\'ops_dat w2_dat[5] = {ops_decl_dat(block, 1, w2_dim, w2_base, w2_d_m, '
         'w2_d_p, &(w2[0][0][0]), "float", "w2time0"), ops_decl_dat(block, 1, w2_dim, '
         'w2_base, w2_d_m, w2_d_p, &(w2[1][0][0]), "float", "w2time1"), '
         'ops_decl_dat(block, 1, w2_dim, w2_base, w2_d_m, w2_d_p, &(w2[2][0][0]), '
         '"float", "w2time2"), ops_decl_dat(block, 1, w2_dim, w2_base, w2_d_m, w2_d_p, '
         '&(w2[3][0][0]), "float", "w2time3"), ops_decl_dat(block, 1, w2_dim, w2_base, '
         'w2_d_m, w2_d_p, &(w2[4][0][0]), "float", "w2time4")}\','
         '\'ops_dat v_dat;\','
         '\'v_dat = ops_decl_dat(block, 1, v_dim, v_base, v_d_m, v_d_p, '
         '&(v[0][0]), "float", "v")\']')
    ])
